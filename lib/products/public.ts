...
import { publicProductWhere } from "@/lib/content/visibility";
...
export async function getPublishedProducts() {
  const products = await db.product.findMany({
    where: publicProductWhere(), select: publicProductSelect,
...
export async function getPublishedProductBySlug(slug: string) {
  const product = await db.product.findFirst({ where: { slug, ...publicProductWhere() }, select: publicProductSelect });
