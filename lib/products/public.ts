...
export async function getPublishedProducts() {
  const products = await db.product.findMany({
    where: { status: ProductStatus.PUBLISHED }, select: publicProductSelect,
...
export async function getPublishedProductBySlug(slug: string) {
  const product = await db.product.findFirst({ where: { slug, status: ProductStatus.PUBLISHED }, select: publicProductSelect });
