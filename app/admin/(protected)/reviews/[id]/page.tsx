...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!review) notFound();
  const localDate = (date: Date | null) => date ? date.toISOString().slice(0, 16) : "";
  const value: ReviewFormValue = { customerName: review.customerName, rating: review.rating, content: review.content, productId: review.productId ?? "", status: review.status, featured: review.featured, pinned: review.pinned, moderationNotes: review.moderationNotes ?? "", publishAt: localDate(review.publishAt), unpublishAt: localDate(review.unpublishAt), sortOrder: review.sortOrder, reviewDate: review.reviewDate?.toISOString().slice(0, 10) ?? "" };
