...
export default async function ProtectedAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="mx-auto grid max-w-[1500px] gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[240px_1fr] lg:px-8">
    <aside className="h-fit rounded-4xl bg-vijaya-dark p-5 text-white shadow-card lg:sticky lg:top-36">
      ...
      <nav aria-label="Admin" className="mt-5"><ul className="space-y-1.5 text-sm font-bold"><li><Link href="/admin" className="block rounded-2xl px-4 py-3 transition hover:bg-white/10 focus-visible:outline focus-visible:outline-2 focus-visible:outline-vijaya-gold">Dashboard</Link></li><li><Link href="/admin/products" className="block rounded-2xl bg-vijaya-red px-4 py-3 transition hover:bg-vijaya-red2 focus-visible:outline focus-visible:outline-2 focus-visible:outline-vijaya-gold">Products</Link></li><li><Link href="/admin/reviews" className="block rounded-2xl px-4 py-3 transition hover:bg-white/10 focus-visible:outline focus-visible:outline-2 focus-visible:outline-vijaya-gold">Reviews</Link></li><li><Link href="/admin/recipes" className="block rounded-2xl px-4 py-3 transition hover:bg-white/10 focus-visible:outline focus-visible:outline-2 focus-visible:outline-vijaya-gold">Recipes</Link></li><li><Link href="/admin/homepage" className="block rounded-2xl px-4 py-3 transition hover:bg-white/10">Homepage</Link></li><li><Link href="/admin/settings" className="block rounded-2xl px-4 py-3 transition hover:bg-white/10">Settings</Link></li></ul></nav>
