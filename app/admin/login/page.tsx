...
export default async function AdminLoginPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <section className="flex min-h-[calc(100vh-12rem)] items-center justify-center px-4 py-14">
    <div className="w-full max-w-md rounded-4xl bg-white p-7 shadow-card sm:p-10">
      ...
      {error && <p role="alert" className="mt-5 rounded-2xl bg-vijaya-pink px-4 py-3 text-center text-sm font-bold text-vijaya-red">Invalid username, email, or password.</p>}
      <form action="/admin/auth/login" method="post" className="mt-7 space-y-5">
        <div><label htmlFor="admin-identifier" className="mb-1.5 block text-sm font-bold">Username or Email</label><input id="admin-identifier" name="identifier" type="text" required maxLength={254} autoComplete="username" className="w-full rounded-2xl border border-vijaya-red/20 bg-white px-4 py-3 focus-visible:outline focus-visible:outline-2 focus-visible:outline-vijaya-red" /></div>
