...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return NextResponse.redirect(new URL("/signup?error=validation", origin), 303);
  try {
    ...
    if (!customer) return NextResponse.redirect(new URL("/signup?error=validation", origin), 303);
    await createCustomerSession(customer.id, request);
    return NextResponse.redirect(new URL("/", origin), 303);
  } catch (error) {
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") return NextResponse.redirect(new URL("/signup?error=exists", origin), 303);
    console.error("Customer signup failed.");
    return NextResponse.redirect(new URL("/signup?error=validation", origin), 303);
  }
}
