...
interface Store { cart:CartLine[]; wishlist:string[]; user:User|null; hydrated:boolean; add:(id:string)=>void; setQty:(id:string,qty:number)=>void; remove:(id:string)=>void; toggleWish:(id:string)=>void; setUser:(user:User|null,remember?:boolean)=>void; toast:(message:string)=>void }
...
export function StoreProvider({children}:{children:React.ReactNode}){
 ...
 const add=useCallback((id:string)=>{if(!productById(id))return;setCart(lines=>{const line=lines.find(x=>x.id===id);return line?lines.map(x=>x.id===id?{...x,qty:x.qty+1}:x):[...lines,{id,qty:1,type:"product"}]});toast("Added to your kitchen!")},[toast]);
 ...
 const toggleWish=useCallback((id:string)=>{if(!productById(id))return;setWishlist(ids=>ids.includes(id)?ids.filter(x=>x!==id):[...ids,id])},[]);
 const setUser=useCallback((value:User|null,remember=true)=>{setUserState(value);localStorage.removeItem("vijaya_user");sessionStorage.removeItem("vijaya_user");if(value)(remember?localStorage:sessionStorage).setItem("vijaya_user",JSON.stringify(value))},[]);
 const value=useMemo(()=>({cart,wishlist,user:userState,hydrated,add,setQty,remove,toggleWish,setUser,toast}),[cart,wishlist,userState,hydrated,add,setQty,remove,toggleWish,setUser,toast]);
