...
interface Store { products:PublicProduct[]; cart:CartLine[]; wishlist:string[]; user:User|null; hydrated:boolean; add:(id:string)=>void; setQty:(id:string,qty:number)=>void; remove:(id:string)=>void; toggleWish:(id:string)=>void; setUser:(user:User|null,remember?:boolean)=>void; toast:(message:string)=>void }
...
export function StoreProvider({children,products}:{children:React.ReactNode;products:PublicProduct[]}){
 ...
 const value=useMemo(()=>({products,cart,wishlist,user:userState,hydrated,add,setQty,remove,toggleWish,setUser,toast}),[products,cart,wishlist,userState,hydrated,add,setQty,remove,toggleWish,setUser,toast]);
