...
export type ProductFormValue = {
  ...
  price: string; compareAtPrice: string; preparationTime: string; servings: string; status: "DRAFT" | "PUBLISHED";
  featured: boolean; preorder: boolean; sortOrder: number; seoTitle: string; seoDescription: string;
};
type Action = (state: ProductFormState, formData: FormData) => Promise<ProductFormState>;
const empty: ProductFormValue = { name: "", slug: "", categoryId: "", shortDescription: "", description: "", imagePath: "", price: "", compareAtPrice: "", preparationTime: "", servings: "", status: "DRAFT", featured: false, preorder: false, sortOrder: 0, seoTitle: "", seoDescription: "" };
...
export function ProductForm({ action, categories, product }: { action: Action; categories: { id: string; name: string }[]; product?: ProductFormValue }) {
  ...
  return <form action={formAction} className="space-y-7" noValidate>
    ...
    <fieldset className="rounded-4xl bg-white p-6 shadow-soft"><legend className="px-2 font-display text-xl font-bold text-vijaya-red">Visibility</legend><div className="mt-3 grid gap-5 sm:grid-cols-2">
      <div><label htmlFor="status" className="mb-1.5 block text-sm font-bold">Status</label><select id="status" name="status" defaultValue={initial.status} className={inputClass}><option value="DRAFT">Draft</option><option value="PUBLISHED">Published</option></select></div>
